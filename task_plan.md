...
查了一下这段代码的来源：**只存在于我们自己这条分支的历史里，`origin/test`（团队/官方基线分支）完全没有**——是在今天早些时候"从 stash 恢复 NS 相关代码"那次提交（`15899866`）里第一次出现的，说明是团队自己早前加的定制，不是 GoEdge 官方/社区版行为。

**决定**：摘掉这段自动逻辑，让这块跟 `origin/test` 保持一致（已确认 `git diff origin/test` 这段代码完全没有差异了）。用户如果真的需要泛域名解析，现在就能做——集群 DNS 设置页面本来就有"自动设置的CNAME记录"（`cnameRecords`）功能，自己手动加一条 `*` 效果是一样的，只是变成显式选择而不是静默强加。
...
**部署后的踩坑 1**：摘掉生成通配符的代码只能阻止*以后*新生成通配符，**不会自动删除数据库里已经存在的历史通配符记录**——部署完之后发现 `w3.test.node`/`gd53.cdn.test.node` 这类随便编的子域名还是能解析出来，一查发现 `test.node` 记录里还留着两条历史 `*` CNAME（两个共用该域名的集群各留了一条）。需要手动把对应集群的 `clusterChange` 任务重置成待处理、等它们在新代码下重新跑一次"多余记录"清理逻辑，才会真正把旧通配符记录删掉。以后再有类似"摘除自动生成逻辑"的改动，记得同时说明需要触发一次重新同步才能清掉历史数据，不是重新部署代码就自动生效。

**部署后的踩坑 2（更重要）**：这段逻辑其实**在代码库里有两份完全独立的拷贝**——`dns_task_executor.go`（后台定时任务，20秒 tick 那条）和 `service_dns_domain.go` 的 `syncClusterDNS`（EdgeAdmin"同步"按钮走的 RPC）各自维护了一份一模一样的"通配符 CNAME"代码块。第一次只删了定时任务那份，验证完看着没问题，结果用户手动点了一下集群 DNS 设置页面的"同步"按钮，通配符又被这第二份拷贝重新生成了一遍。已确认这第二份也是同一个 commit（`15899866`）引入的、`origin/test` 同样没有，一并删掉（commit `721c9093`），并且全仓库搜了一遍确认没有第三份。**教训**：以后删这种"自动生成"逻辑，光找一处删掉不够，要搜全仓库确认有没有重复实现，尤其是"后台定时任务"和"手动触发按钮"这种容易被分别独立实现同一段逻辑的地方。
