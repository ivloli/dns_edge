...
2. **已完成**：实现 GoEdge edgeDNSAPI 兼容接口（`POST /NS*Service/*`），支持智能 DNS 地理分流配置
3. **已完成**：在公司 edgeapi repo 里实现 NS 商业版服务端，让 GoEdge 管理后台具备 EdgeDNS 集群管理能力
...
| 模块 | 状态 |
|------|------|
| `edgedns_provider.go` NS*Service 端点 | ✅ 已完成 |
| `edgedns_auth.go` AccessKey/Token 鉴权 | ✅ 已完成 |
| NSRouteService 地理路由线路 | ✅ 已完成 |
...
## 6. 已完成：edgeapi NS 商业版服务端（Phase 15）
...
| 模块 | 状态 |
|------|------|
| `api_node_services_hook.go` 去除 `!plus` 门控 | ✅ 已完成 |
| `ns_record_dao.go` 补全 CRUD | ✅ 已完成 |
| `ns_route_dao.go` 补全路由查询 | ✅ 已完成 |
| `ns_domain_dao.go` 补全域名查询 | ✅ 已完成 |
| `service_ns_record.go` / `_route` / `_domain` | ✅ 已完成 |
| REST handler 注册 | ✅ 已完成 |
| EdgeCommon import 路径修复（243 文件） | ✅ 已完成 |
| 单元测试 | ⬜ 待做 |
| EdgeAdmin 前端 | ⬜ 待评估 |
...
| P14 | dns-edge edgeDNSAPI 服务端 + 地理路由线路 | ✅ 已完成 |
| P15 | edgeapi NS 商业版服务端（NSRecord/Route/Domain Service） | ✅ 已完成 |
